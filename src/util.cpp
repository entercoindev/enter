...
const char * const BITCOIN_CONF_FILENAME = "enter.conf";
const char * const BITCOIN_PID_FILENAME = "enterd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "enter" is a composite category enabling all Enter-related debug output
            if(ptrCategory->count(string("enter"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EnterCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EnterCore
    // Mac: ~/Library/Application Support/EnterCore
    // Unix: ~/.entercore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EnterCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EnterCore";
#else
    // Unix
    return pathRet / ".entercore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty enter.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override enter.conf
